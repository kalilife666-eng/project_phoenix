...
<head>
    ...
    <title>Disclosure Analysis Tool — Analysis Report</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            <h1>Disclosure Analysis Tool — Analysis Report</h1>
            <p>Disclosure-focused report review</p>
        ...
        html += f"""
            <div class="section">
                ...
                <div class="alert alert-info">
                    {analysis_results.get('overall_assessment', 'No assessment available.').replace('\n', '<br>')}
                </div>
...
"""
        ...
        # State Conduct
        conduct = analysis_results.get("officer_conduct_assessment")
        if conduct:
            css = "alert-high" if conduct["seriousness"] == "HIGH" else "alert-medium"
            html += f"""
            <div class="section">
                <h2>👮 State Conduct Analysis</h2>
                <div class="alert {css}">
                    <strong>{conduct['seriousness']} SERIOUSNESS</strong><br>
                    {conduct['assessment']}
                </div>
                <p><strong>Legal Basis:</strong> {conduct['legal_basis']}</p>
            </div>
"""

        bad_faith = analysis_results.get("bad_faith_assessment")
        if bad_faith:
            css = "alert-high" if bad_faith["level"] == "HIGH" else "alert-medium" if bad_faith["level"] == "MEDIUM" else "alert-info"
            html += f"""
            <div class="section">
                <h2>⚠️ Official Bad Faith Analysis</h2>
                <div class="alert {css}">
                    <strong>{bad_faith['level']} LEVEL</strong><br>
                    Score: {bad_faith['score']}<br>
                    {bad_faith['assessment']}
                </div>
                <p><strong>Legal Basis:</strong> {bad_faith['legal_basis']}</p>
                <p><strong>Findings:</strong></p>
                <ul>
"""
            for finding in bad_faith.get("findings", []):
                html += f"<li><strong>{finding['label']}</strong>: {finding['rationale']}"
                if finding.get("matches"):
                    html += f"<br>Matches: {', '.join(finding['matches'])}"
                html += "</li>"
            html += "</ul>"
            html += f"<p><strong>Charter Sources:</strong> {'; '.join(bad_faith.get('charter_sources', []))}</p>"
            html += f"<p><strong>Reference Sources:</strong> {'; '.join(bad_faith.get('policing_sources', []))}</p>"
            html += "            </div>\n"

        crown_conduct = analysis_results.get("crown_conduct_assessment")
        if crown_conduct:
            css = "alert-high" if crown_conduct["level"] == "HIGH CONCERN" else "alert-medium" if crown_conduct["level"] == "REVIEW REQUIRED" else "alert-info"
            html += f"""
            <div class="section">
                <h2>⚖️ Prosecution Review</h2>
                <div class="alert {css}">
                    <strong>{crown_conduct['level']}</strong><br>
                    Score: {crown_conduct['score']}<br>
                    {crown_conduct['assessment']}
                </div>
                <p><strong>Legal Basis:</strong> {crown_conduct['legal_basis']}</p>
"""
            if crown_conduct.get("findings"):
                ...
                for finding in crown_conduct["findings"]:
                    ...
                    html += "</li>"
                html += "</ul>"
            html += f"<p><strong>Sources:</strong> {'; '.join(crown_conduct.get('sources', []))}</p>"
            html += "            </div>\n"
        ...
        if breaches:
            for breach in breaches:
                ...
                <div class="breach-card">
                    ...
                    <p><em>{breach.get('description', '').replace('\n', '<br>')}</em></p>
...
"""
                        ...
                        if test.get("analysis"):
                            html += "<br>Analysis: " + test["analysis"]
                        if test.get("authorities"):
                            html += "<br>Authorities: " + "; ".join(test["authorities"])
                        html += "                    </div>\n"
...
"""
            for rec in recommendations:
                is_priority = rec.startswith("[PRIORITY]") or rec.startswith("[REMEDY]") or rec.startswith("[CONDUCT]")
        ...
        html += f"""
            <div class="footer">
                <p>Generated by Disclosure Analysis Tool v{os.environ.get("APP_VERSION", "1.0")} — {self.timestamp}</p>
...
"""
    ...
    def generate_text_report(self, analysis_results, document_metadata, output_path):
        ...
        analysis_results = scrub_party_identifiers(analysis_results)
        lines = ["=" * 80, "DISCLOSURE ANALYSIS TOOL — ANALYSIS REPORT",
                 "Disclosure-focused report review",
        ...
        lines.append(f"\nOVERALL ASSESSMENT")
        lines.append("-" * 40)
        lines.append(analysis_results.get("overall_assessment", "No assessment available.").replace('\n', '\n'))

        # State Conduct
        conduct = analysis_results.get("officer_conduct_assessment")
        if conduct:
            lines.append(f"\nSTATE CONDUCT ANALYSIS")
            lines.append("-" * 40)
            lines.append(f"  SERIOUSNESS: {conduct['seriousness']}")
            lines.append(f"  ASSESSMENT:  {conduct['assessment']}")
            lines.append(f"  LEGAL BASIS: {conduct['legal_basis']}")

        bad_faith = analysis_results.get("bad_faith_assessment")
        if bad_faith:
            lines.append(f"\nOFFICIAL BAD FAITH ANALYSIS")
            lines.append("-" * 40)
            lines.append(f"  LEVEL: {bad_faith['level']} (score {bad_faith['score']})")
            lines.append(f"  ASSESSMENT: {bad_faith['assessment']}")
            lines.append(f"  LEGAL BASIS: {bad_faith['legal_basis']}")
            for finding in bad_faith.get("findings", []):
                lines.append(f"  - {finding['label']}: {finding['rationale']}")
                if finding.get("matches"):
        ...
        if crown_conduct:
            lines.append(f"\nPROSECUTION REVIEW")
            lines.append("-" * 40)
            lines.append(f"  LEVEL: {crown_conduct['level']} (score {crown_conduct['score']})")
            lines.append(f"  ASSESSMENT: {crown_conduct['assessment']}")
            lines.append(f"  LEGAL BASIS: {crown_conduct['legal_basis']}")
        ...
        for breach in breaches:
            ...
            lines.append(f"  Description: {breach.get('description', '').replace('\n', '\n')}")
            if breach.get("matched_keywords"):
            ...
            if breach.get("applicable_tests"):
                ...
                for tid, test in breach["applicable_tests"].items():
                    ...
                    if test.get("analysis"):
                        lines.append(f"      Analysis: {test['analysis']}")
                    if test.get("authorities"):
                        lines.append(f"      Authorities: {'; '.join(test['authorities'])}")
