...
def build_canlii_search_url(query):
    encoded_query = urllib.parse.quote(query, safe="")
    return f"https://www.canlii.org/en/search/#search[0][query]={encoded_query}"


def build_canlii_decision_url_from_neutral_citation(citation_text, paragraph=None):
    """
    Build a direct CanLII decision URL from a neutral citation when possible.
    Example supported neutral citation: "2009 SCC 32".
    """
    match = re.search(r"\b((?:19|20)\d{2})\s+([A-Z]{2,6})\s+(\d{1,4})\b", citation_text or "")
    if not match:
        return None

    year, court_code, number = match.groups()
    path = NEUTRAL_CITATION_PATHS.get(court_code.upper())
    if not path:
        return None

    token = f"{year}{court_code.lower()}{int(number)}"
    url = f"https://www.canlii.org/en/{path}/doc/{year}/{token}/{token}.html"
    if paragraph is not None:
        try:
            par_num = int(paragraph)
            if par_num > 0:
                url = f"{url}#par{par_num}"
        except (TypeError, ValueError):
            pass
    return url


def build_authority_reference(authority_text, paragraph=None):
    """
    Normalize a case authority citation into a reference with the best available URL.
    """
    citation = (authority_text or "").strip()
    if paragraph is None and citation:
        para_match = re.search(r"\b(?:para(?:graph)?\.?\s*|¶\s*)(\d{1,4})\b", citation, flags=re.IGNORECASE)
        if para_match:
            paragraph = para_match.group(1)
    direct_url = build_canlii_decision_url_from_neutral_citation(citation, paragraph=paragraph)
    if direct_url:
        return {
            "citation": citation,
            "url": direct_url,
            "location_type": "direct_decision",
            "pinpoint": f"Paragraph {paragraph}" if paragraph else "",
        }

    return {
        "citation": citation,
        "url": build_canlii_search_url(citation),
