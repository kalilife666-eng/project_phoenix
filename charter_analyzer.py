...
TEST_INDICATORS = {
    "officer_duty": [r"\bofficer duty\b", r"\bfailure to inquire\b", r"\bignored? explanation\b", r"\binnocent explanation\b", r"\bstorrey\b"],
    "legal_ignorance": [r"\bmistake of law\b", r"\blegal ignorance\b", r"\bwillful blindness\b", r"\btim\b"],
    ...
    "vagueness": [
        ...
        r"\bphysical evidence\b.{0,40}\balleged property\b",
        r"\balleged property\b.{0,40}\bphysical evidence\b",
    ],
...
class CharterAnalyzer:
    ...
    def _analyze_officer_conduct(self, text, breaches):
        ...
        # "wasnt alleged property thats black and covered with multiple layers of rubber it was physical evidence which had no insulation"
        if ("physical evidence" in text_lower and "insulation" in text_lower) or ("casing" in text_lower and "stripped" in text_lower):
        ...
        material_conflict = (
            ("physical evidence" in text_lower and "alleged property" in text_lower)
            or bool(re.search(r"\b(?:copper|wire)\b.{0,40}\b(?:alleged property)\b|\b(?:alleged property)\b.{0,40}\b(?:copper|wire)\b", text_lower, re.IGNORECASE))
        )
        ...
        if material_conflict or vague_material_language:
            findings.append(
                ...
                "(e.g., physical evidence vs alleged property), indicating failure to assess all available information before arrest/search actions."
            )
            seriousness_level = "HIGH"
        ...
        signal_map = [
            ...
            },
            {
                ...
                "patterns": [
                    ...
                    r"\bappears to be\b",
                    r"\bunknown material\b",
                    r"\bphysical evidence\b.*\balleged property\b|\balleged property\b.*\bphysical evidence\b",
    ...
    def _analyze_grounds_for_arrest(self, text):
        ...
        # Officer Duty to Consider Innocent Explanations (R v Storrey)
        innocent_explanation = bool(re.search(
            r"\b(?:stated|claimed|said|explained)\b.{0,50}\b(?:because|needed to|was just|only)\b",
            text_lower,
            re.IGNORECASE
        ))
        explanation_ignored = innocent_explanation and not bool(re.search(
            r"\b(?:verified|checked|investigated|confirmed|found to be false)\b",
            text_lower,
            re.IGNORECASE
        ))
        if explanation_ignored:
            findings.append("OFFICER DUTY FAILURE: THE RECORD INDICATES AN EXPLANATION WAS PROVIDED BY THE INDIVIDUAL BUT WAS NOT INVESTIGATED BEFORE ARREST (R v STORREY).")

        # Mistake of Law / Legal Ignorance (R v Tim)
        possession_only = bool(re.search(r"\bpossession of\b.{0,30}\b(?:copper|wire|cable|material)\b", text_lower))
        no_stolen_verification = not bool(re.search(r"\b(?:stolen|theft from|obtained by crime)\b", text_lower))
        if possession_only and no_stolen_verification:
            findings.append("LEGAL IGNORANCE / R v TIM: THE ARREST APPEARS BASED ON MERE POSSESSION WITHOUT VERIFYING THE STOLEN STATUS OF THE PROPERTY (MISTAKE OF LAW).")
        ...
        seized_material_mismatch = bool(re.search(
            r"\bseized\b.{0,40}\b(?:wire|physical evidence)\b",
            text_lower,
            re.IGNORECASE,
        )) and (
            ("alleged property" in text_lower and "physical evidence" in text_lower)
            or bool(re.search(r"\bnot present\b.{0,30}\balleged property\b|\balleged property\b.{0,30}\bnot present\b", text_lower, re.IGNORECASE))
        )
        ...
        if len(findings) >= 2:
            conclusion = (
                ...
                'Specifically: (1) Officer Duty Failure: Ignorance or neglect of relevant information '
                '(such as plausible innocent explanations) negates reasonable grounds. (2) Mistake of Law: '
                'Per R v Tim, negligence or legal ignorance (e.g., arresting for non-criminal possession) '
                'does not constitute good faith. (3) Absence of Stolen Property: No material matching '
                'the alleged stolen items was found on the manifest. (4) Observation Failure: '
                'The supporting statement fails to describe a criminal act. (5) Vague / Contradictory Evidence: '
                'Description inconsistency and lack of documentation weaken evidentiary reliability. '
                '(6) Missing Scene Nexus: no connection to a defined crime scene. '
                'This constitutes a breach of the Principles of Fundamental Justice (Section 7) and '
